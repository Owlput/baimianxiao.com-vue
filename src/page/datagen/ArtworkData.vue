...
<template>
        ...
        <form class="h grid grid-cols-1">
            ...
            <div class="authorTriGrid">
                <p>Author ID</p>
                <input v-model="authorId" class="border-2 shadow-md" maxlength="8" minlength="8" />
                <button @click="() => { authorId = randomHex(8) }">Random</button>
            </div>
            <div class="authorTriGrid">
                <p>Permit ID</p>
                <input v-model="permitId" class="border-2 shadow-md" maxlength="8" minlength="8"/>
                <button @click="() => { permitId = randomHex(8) }">Random</button>
            </div>
            <div>
                <p>Tags</p>
                <div v-for="(tag,index) in tags" class="recentGrid">
                    <input v-model="tags[index]" />
                    <button @click="delTag(index)" v-show="!(index == 0 && maxTagsLength == 1)">
            ...
            <div>
                <p>Other Sources</p>
                <div class="contactGrid "><p>Platform</p><p>Address</p></div>
                <div v-for="(source, index) in sourceOther" class="contactGrid">
                    <input v-model="source[0]" class="m-1" />
                    <input v-model="source[1]" class="m-1" /> 
                    <button @click="delSourceOther(index)" v-show="!(index == 0 && maxSourceOtherLength == 1)">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" style="display:block;margin:auto"
                            fill="rgb(200,50,50)" viewBox="0 0 16 16">
                            <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z" />
                            <path
                    ...
                    <button v-show="(index + 1) == maxSourceOtherLength" @click="newSourceOther()">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" style="display:block;margin:auto"
                            fill="rgb(90,170,250)" viewBox="0 0 16 16">
                            <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z" />
                            <path
            ...
            <div class="authorTriGrid">
                ...
                <input v-model="timeOrigin" class="border-2 shadow-md" type="datetime-local" />
            </div>
            <div class="authorTriGrid">
                <p>Time Uploaded on This Site</p>
                <input v-model="timeThis" class="border-2 shadow-md" type="datetime-local" />
            </div>
            <div class="authorTriGrid">
                <p>Views</p>
